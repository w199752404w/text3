...
//			for(uint8_t i=0;i<CFG_CELL_NUM;i++) {
				...
				if((pstPackRVal->fCellTMin >= 15 && /*pstPackRVal->afCellU[i]*/pstPackRVal->fCellUMin <= g_stCfg.stLocal.ausSocECaliU[0] * 0.001 && pstPackRVal->fPackCur > -100 && pstPackRVal->fPackCur < -5)
					|| (pstPackRVal->fCellTMin < 15 && pstPackRVal->fCellTMin >= 5 && /*pstPackRVal->afCellU[i]*/pstPackRVal->fCellUMin <= 2700 * 0.001 && pstPackRVal->fPackCur > -100 && pstPackRVal->fPackCur < -5)
					|| (pstPackRVal->fCellTMin < 5 && /*pstPackRVal->afCellU[i]*/pstPackRVal->fCellUMin <= 2550 * 0.001 && pstPackRVal->fPackCur > -100 && pstPackRVal->fPackCur < -5)) {
					if(s_usTick1/*[i]*/ * g_stCfg.stLocal.usCyclePeriod < 5000) {
						s_usTick1/*[i]*/++;
					}
				} else {
...
//			for(uint8_t i=0;i<CFG_CELL_NUM;i++) {
				...
				if((pstPackRVal->fCellTMin >= 15 && /*pstPackRVal->afCellU[i]*/pstPackRVal->fCellUMin <= g_stCfg.stLocal.ausSocECaliU[1] * 0.001)
					|| (pstPackRVal->fCellTMin < 15 && pstPackRVal->fCellTMin >= 5 && /*pstPackRVal->afCellU[i]*/pstPackRVal->fCellUMin <= 2400 * 0.001)
					|| (pstPackRVal->fCellTMin < 5 && /*pstPackRVal->afCellU[i]*/pstPackRVal->fCellUMin <= 2300 * 0.001)) {
					if(s_usTick1/*[i]*/ * g_stCfg.stLocal.usCyclePeriod < 5000) {
						s_usTick1/*[i]*/++;
					}
				} else {
...
/* trigger: Temperature =10? and Cell volt.<2.25V or Temperature <10? and Cell volt.<2.10V, BMS turn off  */
		{
			static uint8_t s_ucTick = 0;
			static uint16_t s_ucTick1 = 0;
			if((pstPackRVal->fCellTMin >= 10 && pstPackRVal->fCellUMin < g_stCfg.stLocal.usCellUVTVThr1 * 0.001)
				|| (pstPackRVal->fCellTMin < 10 && pstPackRVal->fCellUMin < 2100 * 0.001)) {
...
//						pstPackRVal->fPackSoc = 0;
	...
	/* trigger: Temperature =10? and Cell volt.<2.35V or Temperature <10? and Cell volt.<2.15V */
	{
		static uint8_t s_ucTick = 0;
		if((pstPackRVal->fCellTMin >= 10 && pstPackRVal->fCellUMin < g_stCfg.stLocal.usCellUVTVThr2 * 0.001 && pstPackRVal->fCellUMin > g_stCfg.stLocal.usCellUVTVThr1 * 0.001)
			|| (pstPackRVal->fCellTMin < 10 && pstPackRVal->fCellUMin < 2150 * 0.001 && pstPackRVal->fCellUMin > 2100 * 0.001)) {
