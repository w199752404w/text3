...
//		}
		...
		ptc_eco_set(g_ucAlmLevel/*g_ucAlmLeve3*/, E_PTC_ECO_CV14_48, 13);
		{
		...
		ptc_eco_set(g_ucAlmLevel/*g_ucAlmLeve3*/, E_PTC_ECO_CV27_48, 26);
		{
			static uint8_t aucBuf = 0;
			static uint8_t s_uctick = 1, uc_tick = 0;
			for(uint8_t i=s_uctick; i<65; i++){
...
//			}
		...
		if(/*pstPackRVal->ucDsgEn == 0x55 || pstPackRVal->ucDsgForceEn == 0x55*/g_stAfe.uRam.stCode.DSG_FET == 1) {
			ptc_eco_set(0, E_PTC_ECO_CV43_4, 42);
		} else {
			ptc_eco_set(1, E_PTC_ECO_CV43_4, 42);
		}
		if(/*pstPackRVal->ucChgEn == 0x55 || pstPackRVal->ucChgForceEn ==0x55*/g_stAfe.uRam.stCode.CHG_FET == 1) {
	...
	if(g_stCfg.stCom.ucCanPtcType == 1 || g_stCfg.stCom.ucCanPtcType == 2 || g_stCfg.stCom.ucCanPtcType == 5 || g_stCfg.stCom.ucCanPtcType == 6 || g_stCfg.stCom.ucCanPtcType == 7) {
