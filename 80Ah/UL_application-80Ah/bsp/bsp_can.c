...
* Description		: CAN的GPIO configuration
* Input					: None
 * Output				: None
 */
...
static void CAN0_Mode_Config(uint32_t uiBaud) {
    ...
    can_parameter.auto_wake_up = DISABLE;//ENABLE 2024.12.23 dgx
    can_parameter.auto_retrans = DISABLE; //dgx
...
{
    /* 禁用CAN错误中断 */
    can_interrupt_disable(CAN0, CAN_INT_ERRN); // 禁用错误通知中断
    can_interrupt_disable(CAN0, CAN_INT_ERR);  // 禁用错误捕获中断
    
//    /* 清除错误标志位 */
//    can_flag_clear(CAN0, CAN_ERROR_BITRECESSIVE);
}
//void can_disable_error_interrupt(void) {
//    /* 假设CANx为你要操作的CAN实例，例如CAN0 */
//    #define CANx CAN0
// 
//    /* 禁用CAN总线错误中断 */
//    CANx->INTEN &= ~CAN_INTEN_BOEIE;
//	CANx->
// 
//    /* 禁用CAN奇偶错误中断 */
//    CANx->INTEN &= ~CAN_INTEN_PEIE;
// 
//    /* 禁用CAN总线偏离中断 */
//    CANx->INTEN &= ~CAN_INTEN_WEIE;
// 
//    /* 禁用CAN仲裁丢失中断 */
//    CANx->INTEN &= ~CAN_INTEN_EWEIE;
// 
//    /* 禁用CAN传输FIFO溢出中断 */
//    CANx->INTEN &= ~CAN_INTEN_EPEIE;
// 
//    /* 禁用CAN接收FIFO溢出中断 */
//    CANx->INTEN &= ~CAN_INTEN_EPSIE;
// 
//    /* 禁用CAN passive error interrupt */
//    CANx->INTEN &= ~CAN_INTEN_PSEIE;
// 
//    /* 禁用CAN arbitration lost interrupt */
//    CANx->INTEN &= ~CAN_INTEN_ALIE;
// 
//    /* 禁用CAN error warning interrupt */
//    CANx->INTEN &= ~CAN_INTEN_WIE;
// 
//    /* 禁用CAN transition from error-active to error-passive interrupt */
//    CANx->INTEN &= ~CAN_INTEN_EAIE;
//}
