...
//		
//		MEM_FlashRead((((g_usHDataIdx + 5120 - 1) % 5120) * 2 + eHisDataPStart) * PAGE_SIZE, (uint8_t*)readBuffer, sizeof(HIS_DATA_S));
//		
//		if(memcmp((g_usHDataIdx * 2 + eHisDataPStart) * PAGE_SIZE,(uint8_t*)readBuffer,sizeof(HIS_DATA_S)) != 0) {
//			
//		}
//	}
...
bool his_data_clear(void) {
	HIS_DATA_S stData2 = {0};
	his_data_read(4999, &stData2);
	if(stData2.dt ==0xFFFFFFFF || stData2.dt == 0) {
		for(uint16_t i = eHisDataPStart / 16; i < g_usHDataIdx/8; i++) {
...
bool his_log_clear(void) {
	HIS_LOG_S stLog1 = {0};
	his_log_read(7999, &stLog1);
	if(stLog1.dt == 0xFFFFFFFF || stLog1.dt == 0) {
		for(uint16_t i = eHisLogPStart / 16; i < g_usHLogIdx/4; i++) {
			BSP_W25Qx_Erase_Sector(i);
			WDG_DONE_H;
			WDG_DONE_L;
			gd32_wdgt_feed_dog(EWdgType_fwdg);	//feed dog
