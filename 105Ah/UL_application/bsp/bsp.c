...
void BSP_Init(void) {	 
	...
	MX_GPIO_Init();                                           //Initialize GPIO port  2024/12/20 dgx
	UART_Init(115200, USART_WL_8BIT, USART_STB_1BIT, USART_PM_NONE);
  UART_Init_Dma_Receive(); 
	CAN0_Init(250);
	ADC_Init();                                               //Initialize ADC 	
	TIM_Init();                                               //Initialize TIM
//EXTI0_REST_Init();                                        //Initialize EXTI0 
//	EXTI5_SW2_Init();                                       //Initialize EXTI5
//	EXTI2_SW_Init();                                         //Initialize EXTI2
//	EXTI15_CHG_Init();
...
void BSP_WK_Detection(void) {
	...
	ucCnt = 0;
	while(SW_WK_READ == 0 || SW2_WK_READ == 0) {
		ucCnt++;
		delay_1ms(1);
		if(ucCnt > 50) {//99
	...
	if(SW_WK_READ == 1 && SW2_WK_READ == 1 && ucCnt > 0) {
		g_stCfg.usGoRun = 2;
	}

	if(g_stCfg.usGoRun == 0 || g_stCfg.usGoRun == 2) {
//		g_stCfg.usGoRun = 3;
//	  cfg_save();      //dgx 20250217  siji
//		while(1);
		BSP_DEBUG("shutdown\r\n");
		delay_1ms(1000);
		System_Sleep();
