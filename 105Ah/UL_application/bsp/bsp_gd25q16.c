/*******************************************************************************
* GD25Q16  16 M-Bit 2M Byte 
* 
* 256 Byte One Page
* 
*******************************************************************************/
#include "bsp_gd25q16.h"
#include <stdio.h>
#include <string.h>
#include "systick.h"
...
void SPI_GPIO_Init() {
  ...
  gpio_init(GPIOA, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_6 | GPIO_PIN_7); /* SPI GPIO config: CS/PA3, SCK/PA4, MOSI/PA6, HOLD/PA7 */
	gpio_init(GPIOA, GPIO_MODE_IN_FLOATING, GPIO_OSPEED_50MHZ, GPIO_PIN_5);                                   /* SPI GPIO config: MISO/PA5 */
	gpio_init(GPIOC, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_4);                                        /* SPI GPIO config: WP/PC4 */
...
//uint32_t tickstart = HAL_GetTick();
	...
	W25Qx_Disable();
	delay_1ms(45);                         //Simulate SPI, must
	/* Wait the end of Flash writing */
	while(BSP_W25Qx_GetStatus() == W25Qx_BUSY) {
	}
	/* Check for the Timeout */
