...
void USART2_IRQHandler(void) {
	...
	if(RESET != usart_interrupt_flag_get(USART2, USART_INT_FLAG_RBNE)){
		/* receive data */
//	  g_stUsart.aucRBuf[g_stUsart.usRLen] = usart_data_receive(USART2);
//		g_stUsart.usRLen++;
//		if(g_stUsart.usRLen >= USART_MLEN) {
//			USART2_UgFlag = 1;
//			USART2_RxFlag = 1;
//		}
	...
	if(RESET != usart_interrupt_flag_get(USART2, USART_INT_FLAG_IDLE)) {
		...
		if(USART2_UgFlag == 0){
		  USART2_RxFlag = 1;
			...
			if(g_stUsart.usRLen > 1020 && g_stUsart.usRLen < 1200){
				USART2_RxFlag = 1;
			}
			if(g_stUsart.aucRBuf[0] == 0x04 && g_stUsart.aucRBuf[1] == 0x01 && g_stUsart.aucRBuf[2]==0x00 && g_stUsart.aucRBuf[3] == 0xFF){
			  USART2_RxFlag = 1;
		  }
		}
	}
...
void CAN0_RX1_IRQHandler(void) {
	...
	if((canRBuf.uiId & 0x1FFFFF) != 0x000E00) {
	...
	for(uint8_t i = 0; i < 4; i++) {
		if(g_auiIEMI_IDs[i] == uiID) {
			break;
		}
		if(g_auiIEMI_IDs[i] == 0) {
			g_auiIEMI_IDs[i] = uiID;
			s_uci = i;
			break;
		}
	}
...
//				}
	...
	if(g_stPrl.ucSelfId != ucSelfId) {
		memcpy(g_stLocalArrayRVal.astPackRVal + ucSelfId, g_stLocalArrayRVal.astPackRVal + g_stPrl.ucSelfId, sizeof(LOCAL_PACK_RVAL_S));
		g_stPrl.ucSelfId = ucSelfId;
	}
	if(ucDevNum > 2) {
