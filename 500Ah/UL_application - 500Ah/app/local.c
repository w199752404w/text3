...
void local_info_refresh(void) {
	...
	float fDeltaAH = (g_stAfe.stRamApp.fCDATA * 0.001 - 0.04) * g_stCfg.stLocal.usCyclePeriod / 1000 / 3600;	//-0.05A, becuase of including bms self energe using
	for(uint8_t i=0;i<CFG_CELL_NUM;i++) {
		pstPackRVal->afCellLeftAH[i] += fDeltaAH;
	}
	
	if(g_stCfg.stLocal.usDesignAH < 40) { // 2024/12/10 dgx
		g_stCfg.stLocal.usDesignAH = 400;
		cfg_save();
	}
...
//		if((pstPackRVal->fCellTMin >= 10 && pstPackRVal->afCellU[i] <= (g_stCfg.stLocal.ausSocECaliU[2]) * 0.001)
//			|| (pstPackRVal->fCellTMin < 10 && pstPackRVal->afCellU[i] <= (g_stCfg.stLocal.ausSocECaliU[2] - 150) * 0.001)) {
//			if(s_usTick[i] * g_stCfg.stLocal.usCyclePeriod < 5000) {
//				s_usTick[i]++;
//			}
//			} else {
//				s_usTick[i] = 0;
...
//						pstPackRVal->fPackSoc = 0;
	...
	/* new trigger: Low Temper. 0℃ 2024.12.28 */
	{
		...
		if(round(pstPackRVal->fCellTMin) > g_stCfg.stLocal.sCellUCTTVThr1 && GET_ALM0_CODE(12) && GET_ALM1_CODE(12)) {
			b_LowTemp = false;
			RESET_ALM0_CODE(12);
			RESET_ALM1_CODE(12);
		}
...
void local_mos_ctl(void) {
	...
	if(((pstPackRVal->ucDsgEn == 0x55 && pstPackRVal->ucDsgForceEn == 0)|| pstPackRVal->ucDsgForceEn == 0x55) && g_stAfe.uRam.stCode.DSG_FET == 0) {
		...
		if(ucCnt * g_stCfg.stLocal.usCyclePeriod < 400) {
			PRE_DSG_H;
			ucCnt++;
		} else {
