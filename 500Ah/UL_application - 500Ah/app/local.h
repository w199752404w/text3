...
The limit case battery voltage is 48V, and the external load resistance shall not be greater than 16¦¸,
The pre-play circuit resistance is 25.5 ohms, the total series resistance is 41.5 ¦¸, and the minimum current is not less than 1150mA,
...
#define LOCAL_CUR_RESIST	(0.002/60)		/* unit: ¦¸ */
...
typedef enum {
	...
	eBStatLowTemp = 0x18,         /* Triggered when the temperature is below 0¡ãC */
...
typedef struct {
	...
	float afCellT[CFG_TMP_NUM];				/* Cell temperature, unit: ¡ãC */
	...
	float fMosT;											/* MOS temperature, unit: ¡ãC */
	...
	float fCellTMax;									/* Maximum cell temperature, unit: ¡ãC */
	float fCellTMin;									/* Minimum cell temperature, unit: ¡ãC */
...
typedef struct {
	...
	float fPackTMax;	/* The highest cell temperature in a battery pack, unit: ¡ãC */
	float fPackTMin;	/* The lowest cell temperature in a battery pack, uint: ¡ãC */
