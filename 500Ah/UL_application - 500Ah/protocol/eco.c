...
//			uint32_t ucSelfId:2;
	...
	if(uHeader.stCanId.ucDevAddr >= PRL_MAX_NODE_NUM) {
		return;
	}
...
//		aucCanSBuf[12] = '0';//4
		...
		aucCanSBuf[11] = g_stCfg.stLocal.ucSVerAgree;//'A'
		aucCanSBuf[12] = '-';
		aucCanSBuf[13] = 'G';
		aucCanSBuf[14] = 'e';
		aucCanSBuf[15] = 'n';
...
void eco_write_reg(uint16_t usRegAddr, uint16_t usData) {
	...
	case 0xC6: 	//fCDATACaliA_CHG
	    ...
	    g_stCfg.stAfe.fCDATACaliA_CHG = (float)usData *1000 / g_stAfe.stRamApp.fCDATA * g_stCfg.stAfe.fCDATACaliA_CHG;
	...
	case 0xC7: 	//fCDATACaliA_DSG 
		if(usData && g_stAfe.stRamApp.fCDATA < 0){
	    g_stCfg.stAfe.fCDATACaliA_DSG = (float)usData * 1000 / fabs(g_stAfe.stRamApp.fCDATA) * g_stCfg.stAfe.fCDATACaliA_DSG;
